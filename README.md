...
《柚子社将领包》的长期开发过程中使用了 **OpenAI ChatGPT、Codex 及多代 GPT 系列模型**进行技术辅助，包括但不限于：
...
随着项目规模扩大，制作组也使用 Codex / GPT 模型协助重构了部分早期重复度较高的算法和脚本逻辑。

所有准备发布的 AI 生成或修改内容，仍由人类制作组成员负责审查、测试、调整并决定是否采用。

**项目设计、角色与机制取舍、版本发布、维护以及最终责任均由人类制作组承担。**

<details>
<summary><strong>GPT 模型进化史（开发彩蛋 / 不完全版）</strong></summary>

项目负责人长期使用 GPT 系列模型。

早期 GPT 更多被用于实验数据处理、文字整理和简单辅助；随着模型能力与项目复杂度同时增长，其用途逐渐扩展至翻译、UI 设计、机制讨论、代码审查，以及大规模算法逻辑重构。

> 从处理一条标准曲线，到重构一整个 HOI4 Mod 的算法逻辑。

这里只作为项目开发史彩蛋记录，并不意味着公开仓库中的每段代码都由 AI 生成，也不试图穷举开发过程中使用过的每一个模型版本。
...
AI-assisted software engineering, code refactoring, debugging, documentation and technical consultation.

> 制作组内部彩蛋称呼：**GPT先生 / GPT院士**  
> 该称呼仅为项目内部玩梗，不代表任何现实学术头衔或机构会员资格。
